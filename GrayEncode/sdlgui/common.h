...
std::wstring  file_dialog(const std::vector<std::pair<std::wstring, std::wstring>> &filetypes, bool save);
...
public:
  ...
  inline float getAngleWith(const Vector2<T>& b) const
  {
	...
	float sqrtF = sqrtf( static_cast<float>((float)1.0 - (float)tmp * (float)tmp) / (float)tmp);

    return atanf( static_cast<float>(sqrtF * math::RADTODEG64 ) );
  }
